...
#ifdef DEBUG_EHCI
#define s_printf(fmt, ...)   svc_printf(fmt, ## __VA_ARGS__)
#else
#define s_printf(fmt, ...)   do {} while (0)
#endif
...
static int ehci_init(void)
{
	...
	ehci->ctrl_buffer = mem_usbctrlrequest;
	ehci->command = 0;
...
// off_callback_hand is used when you disables ehcmodule access
	...
	if (flags & STS_PCD) {
		...
		for (n = 0; n < 4; n++) {
			...
			if (n == current_port) {
				...
				if(n!=current_port) {
					ehci_writel(PORT_OWNER, &ehci->regs->port_status[n]);
				}
			}
		}
	}
...
static s32 interrupt_callback_hand(u32 flags)
{
	s32 n;
	s32 ret = 1; // do nothing
	u32 temp;
...
int ehci_do_urb(struct ehci_device *dev, struct ehci_urb *urb)
{
	...
	Swi_uMemcpy(&cached_qh, qh, 96/*sizeof(struct ehci_qh)*/);
	...
	Swi_Memcpy(qh, &cached_qh, 96);

	ehci_irq_working_callback_stage1(interrupt_callback_hand, usb_timeout);
	...
	retval = ehci_irq_working_callback_stage2();
...
int ehci_reset_port_old(int port)
{
	...
	{ 
		...
		retval = handshake(status_reg, status_reg, PORT_RESET, 0, 2*1000);
...
void ehci_acquire_port(int port)
{
	u32 __iomem	*status_reg = &ehci->regs->port_status[port];
	u32 status = ehci_readl(status_reg); 
...
int ehci_reset_usb_port(int port)
{
	...
	{       
		...
		retval = handshake(status_reg, status_reg, PORT_RESET, 0, 5*1000);
...
{	
	...
	if (retval < 0) {
		for (i = 0; i < 3; i++) {
			...
			if (retval < 0) {
				...
				if (dev->desc.idProduct==0x2077 && dev->desc.idVendor==0x950b) //usblan
				{
	...
	if (retval < 0) {
		for (i = 0; i < 3; i++) {
			...
			if (ehci_reset_usb_port(port) == -1119)
				return -1119;
			ehci_acquire_port(port);
			ehci_msleep(100);
			ehci_reset_usb_port(port);
	...
	do {
		...
		if (retval < 0) {
			...
			s_printf("USB_REQ_SETADDRESS ok: %d\n",cnt);
		...
		if (retval < 0) {
			...
			s_printf("ok checking USB_REQ_GETDESCRIPTOR\n");
...
int ehci_discover(void)
{
	...
	for (i = current_port; i < 1 + current_port; i++) {
 ...
 void * ehci_fd_to_dev(int fd)
{
	...
	/* Return always device[current_port] */
	return &ehci->devices[current_port];
}
#define g_ehci #error
